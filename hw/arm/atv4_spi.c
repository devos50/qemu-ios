...
static int apple_spi_word_size(ATV4SPIState *s)
{
...
static void apple_spi_update_xfer_tx(ATV4SPIState *s)
{
...
static void apple_spi_update_xfer_rx(ATV4SPIState *s)
{
...
static void apple_spi_update_irq(ATV4SPIState *s)
{
    uint32_t irq = 0;
    uint32_t mask = 0;
...
static void apple_spi_update_cs(ATV4SPIState *s)
{
...
{
    ATV4SPIState *s = ATV4_SPI(opaque);
    if (level) {
...
static void apple_spi_run(ATV4SPIState *s)
{
    uint32_t tx;
    uint32_t rx;
...
static uint64_t atv4_spi_read(void *opaque, hwaddr addr, unsigned size)
{
    ATV4SPIState *s = ATV4_SPI(opaque);
...
static void atv4_spi_write(void *opaque, hwaddr addr, uint64_t data, unsigned size)
{
    ATV4SPIState *s = ATV4_SPI(opaque);
...
static const MemoryRegionOps spi_ops = {
    .read = atv4_spi_read,
    .write = atv4_spi_write,
    .endianness = DEVICE_NATIVE_ENDIAN,
};

static void atv4_spi_reset(DeviceState *d)
{
    ATV4SPIState *s = (ATV4SPIState *)d;
...
static void atv4_spi_realize(DeviceState *dev, struct Error **errp)
{
    ATV4SPIState *s = ATV4_SPI(dev);
...
static void atv4_spi_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);
    dc->realize = atv4_spi_realize;
    dc->reset = atv4_spi_reset;
}

static const TypeInfo atv4_spi_info = {
    .name          = TYPE_ATV4_SPI,
    .parent        = TYPE_SYS_BUS_DEVICE,
    .instance_size = sizeof(ATV4SPIState),
    .class_init    = atv4_spi_class_init,
};

static void atv4_spi_register_types(void)
{
    type_register_static(&atv4_spi_info);
}

type_init(atv4_spi_register_types)
                            ...
                            unsigned size)
{
    ...
    WITH_QEMU_LOCK_GUARD(&s->mutex) {
        ...
        default:
            hw_error("AIC: Write to unspported reg 0x%08x cpu %u: 0x%x\n", addr, o->cpu_id, val);
            break;
        }
    }
}
    ...
    WITH_QEMU_LOCK_GUARD(&s->mutex) {
        ...
        default: {
            if (addr == 0x20) {
                ...
                hw_error("AIC: Read from unspported reg 0x%08x cpu: %u\n", addr, o->cpu_id);
            }
        }
        }
    }
...
static void apple_aic_realize(DeviceState *dev, struct Error **errp)
{
    ...
    for (i = 0; i < s->numCPU; i++) {
        ...
        cpu->aic = s;
        cpu->cpu_id = i;
        memory_region_init_io(&cpu->iomem, OBJECT(dev), &apple_aic_ops, cpu, TYPE_APPLE_AIC, 0x6000);
...
AppleAICState *apple_aic_create()
{
    DeviceState  *dev;
    AppleAICState *s;
